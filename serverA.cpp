...
int main()
{
    unordered_map<string, vector<vector<int>>> username_intervals_map = read_input("a.txt");
    // Print hashmap
    ...
    if (bind(backend_A_socket, (struct sockaddr *)&backend_A_address, sizeof(backend_A_address)) == -1)
    {
    ...
    if (sendto(backend_A_socket, user_list.c_str(), user_list.length(), 0, (struct sockaddr *)&main_server_address, sizeof(main_server_address)) == -1)
    {
    ...
    while (true)
    {
        ...
        if (recvfrom(backend_A_socket, buffer, max_buffer_size, 0, (struct sockaddr *)&main_server_response_address, &main_server_response_address_length) == -1)
        {
        ...
        if (sendto(backend_A_socket, result.c_str(), result.length(), 0, (struct sockaddr *)&main_server_address, sizeof(main_server_address)) == -1)
        {
        ...
        if (sendto(backend_A_socket, confirmation.c_str(), confirmation.length(), 0, (struct sockaddr *)&main_server_address, sizeof(main_server_address)) == -1)
        {
